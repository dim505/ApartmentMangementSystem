...
export default class AddPropertyAddressSuggest extends Component {
        state = {checked: true, SelectedCheckboxID: "", SuggestedAddresses: []}
        ...
        handleCheckBoxClick = (locationId) => { 
            ...
            if (locationId === this.state.SelectedCheckboxID && this.state.checked === true ) {
               
               this.setState({checked: false}, () =>  this.props.SuggAddrChecked(this.state.checked))
        ...
        FilterAdddress = (locationId) => {
           ...
           window.SuggestedAddress = window.SuggestedAddress.filter( (address) => 
       ...
       render() {
         ...
         if (this.props.SuggestedAddresses <= 0) {
          SuggestedAddr.push(<p>Please start typing a street address to get some suggestions</p>); 
         }
		 //checks if API call has been made and finds anything 
           ...
           window.SuggestedAddresses = window.SuggAddrMatchFilter
              ...
              if (this.props.SuggestedAddresses
                .filter(Address => Address.matchLevel === "houseNumber").length <= 0) {
                  SuggestedAddr.push(<p>No suggested Address. Try starting with a street address</p>);
                
              } else {
				 //filters house level matches to max of 10 then builds out component to house address
                this.props.SuggestedAddresses
                .filter(Address => Address.matchLevel === "houseNumber").slice(0,10)
                .map(Address =>
                  SuggestedAddr.push(
                    <Paper elevation={3}>
