...
export default class ReceiptMainTable extends Component {
  ...
  OpenImageModal(id) {
    ...
    ReceiptFilterdImg = ReceiptFilterdImg.filter(
      (Receipt) => Receipt.id === id
    );
    var objectURL = this.CreateImageUrl(
      ReceiptFilterdImg[0].image,
      ReceiptFilterdImg[0].contentType
    );
    ReceiptFilterdImg.imageUrl = objectURL;

    this.setState({
      OpenImageModal: true,
      ReceiptFilterdImg: ReceiptFilterdImg,
    });
  }
